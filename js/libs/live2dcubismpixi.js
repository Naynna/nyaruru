    ...
    var Model = (function (_super) {
        ...
        function Model(coreModel, textures, animator, physicsRig, userData, groups) {
            ...
            for (var m = 0; m < _this._meshes.length; ++m) {
                ...
                _this._meshes[m] = new PIXI.SimpleMesh(textures[_this._coreModel.drawables.textureIndices[m]], _this._coreModel.drawables.vertexPositions[m], uvs, _this._coreModel.drawables.indices[m], PIXI.DRAW_MODES.TRIANGLES);
                _this._meshes[m].name = _this._coreModel.drawables.ids[m];
                _this._meshes[m].scale.y *= -1;
                _this._meshes[m].state.culling = !LIVE2DCUBISMCORE.Utils.hasIsDoubleSidedBit(_this._coreModel.drawables.constantFlags[m]);
    ...
    var MaskSpriteContainer = (function (_super) {
            ...
            for (var m = 0; m < pixiModel.meshes.length; ++m) {
                if (_maskCounts[m] > 0) {
                    ...
                    for (var n = 0; n < _maskRelationList[m].length; ++n) {
						...
						var uvs = coreModel.drawables.vertexUvs[meshMaskID].slice(0, coreModel.drawables.vertexUvs[meshMaskID].length);
						for (var v = 1; v < uvs.length; v += 2) {
							uvs[v] = 1 - uvs[v];
						}
                        var maskMesh = new PIXI.SimpleMesh(pixiModel.meshes[meshMaskID].texture, pixiModel.meshes[meshMaskID].vertices, uvs, coreModel.drawables.indices[meshMaskID], PIXI.DRAW_MODES.TRIANGLES);
                        ...
                        maskMesh.state.culling = pixiModel.meshes[meshMaskID].state.culling;
                        maskMesh.state.clockwiseFrontFace = true;
                        maskMesh.filters = [_this._maskShader];
						maskMesh.updateTransform = function() {};
                        newContainer.addChild(maskMesh);
                    }
					newContainer.updateTransform = function() {};
                    _this._maskMeshContainers.push(newContainer);
                    var newTexture = PIXI.RenderTexture.create(Graphics.width, Graphics.height);
        ...
        MaskSpriteContainer.prototype.update = function (appRenderer) {
            for (var m = 0; m < this._maskSprites.length; ++m) {
				this._maskMeshContainers[m].children.forEach((ch) => { 
					ch.calculateBounds(); 
				});
                appRenderer.render(this._maskMeshContainers[m], this._maskTextures[m]);
            }
        };
		...
		MaskSpriteContainer.prototype.createRenderTexture = function(width, height) {
    ...
    var ModelBuilder = (function () {
        ...
        ModelBuilder.prototype.buildFromModel3Json = function (loader, model3Obj, callbackFunc) {
            ...
            if (typeof (model3Obj.data['FileReferences']['Moc']) !== "undefined")
                loader.add('moc', modelDir + model3Obj.data['FileReferences']['Moc'], { xhrType: PIXI.LoaderResource.XHR_RESPONSE_TYPE.BUFFER });
            ...
            if (typeof (model3Obj.data['FileReferences']['Physics']) !== "undefined")
                loader.add('physics', modelDir + model3Obj.data['FileReferences']['Physics'], { xhrType: PIXI.LoaderResource.XHR_RESPONSE_TYPE.JSON });
            if (typeof (model3Obj.data['FileReferences']['UserData']) !== "undefined")
                loader.add('userdata', modelDir + model3Obj.data['FileReferences']['UserData'], { xhrType: PIXI.LoaderResource.XHR_RESPONSE_TYPE.JSON });
